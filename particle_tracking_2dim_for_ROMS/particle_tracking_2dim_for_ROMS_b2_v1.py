...
from myfunc.nc_operate import inocean2

################### 命令行参数  ##################
# workdir = sys.argv[1] 
# release_time = (int(sys.argv[2])-1)*86400 + 0 # 释放的初始时刻, 单位秒
workdir = 'C:\\Users\\XUSHENG\\Desktop\\particle_tracking_results\\plant\\'
release_time = 0
############################ 控制参数 ##############################
## 工作目录
os.chdir(workdir)
...
if __name__ == '__main__':
    ...
    #     a4 = np.load(str(3*interval)+'_'+str(N)+'.npy')    

    #     a = np.concatenate((a1,a2,a3,a4),axis=0)
    # else: # 笔记本
    #     release(0,N)
    #     a = np.load('0_'+str(N)+'.npy')

    release(0,N)
    a = np.load('0_'+str(N)+'.npy')

    for i in range(a.shape[2]):
        np.savetxt(str(int(a[0,2,i]))+'_'+'.dat',a[:,:,i])

    T2 = time.perf_counter()
    print()
    print('共计耗时: ',(T2-T1)/3600,'小时')
