    # 8.0 更新说明
# 使用一次性读取所有数据到内存的方式, 减少反复从磁盘读取的时间, 一年的数据大小约为4GB, 内存还顶得住.
# 预设部分变量的长度, 避免使用append方式来反复修改变量长度的低效做法.

# 多进程之间内存不能相互访问... 如何解决? 
# 1. 使用大内存服务器, 每个进程占据4G左右, 64G开12个无压力
# 2. 不load一整年的内存, 将指定时间段的数据集成
# 3. 解决变量共享问题, 一劳永逸

# 优化后, 现在是3小时/40000个粒子*30天, 也就是说, 跑百万粒子三天大概需要8小时左右, 单进程相比之前提高了将近百倍的速度.
...
################### 命令行参数  ##################
workdir = sys.argv[1] 
release_time = int(sys.argv[2])*86400 + 0 # 释放的初始时刻, 单位秒
############################ 控制参数 ##############################
## 工作目录
os.chdir(workdir)
...
content2 = Dataset(flowfield_prefix+'his_0001.nc')
...
time_rho0 = time_rho[0] # 读取到u_rho和v_rho索引的初始时刻
#####################################################################
...
def save_ini_final():
    ...
    ini = np.zeros([N,2])
    # final
    with open('final.dat','w',encoding='utf-8') as f:
        for i in range(N):
            each_particle_data = linecache.getline('particle_'+str(i+1)+'.dat',timestep_numbers+1)
            f.write(each_particle_data)
    f.close()
    # ini
    np.savetxt('initial.dat',ini_particle_matrix[:,0:2])
...
#     p1 = Process(target=release,args=(0,500000))
#     p2 = Process(target=release,args=(500000,100000))
#     p1.start()
#     p2.start()
#     p1.join()
#     p2.join()
